    ...
    parent.postMessage(
      {
        pluginMessage: {
          type: "copied",
          text: document.getElementById("hexCode").value,
        },
      },
      "*"
    );
  ...
  parent.postMessage(
    {
      pluginMessage: {
        type: "addToCanvas",
        text: document.getElementById("hexCode").value,
        value: hexToRgb(document.getElementById("hexCode").value),
      },
    },
    "*"
  );
    ...
    parent.postMessage(
      {
        pluginMessage: {
          type: "copied",
          text: document.getElementById("hexCode2").value,
        },
      },
      "*"
    );
  ...
  parent.postMessage(
    {
      pluginMessage: {
        type: "addToCanvas",
        text: document.getElementById("hexCode2").value,
        value: hexToRgb2(document.getElementById("hexCode2").value),
      },
    },
    "*"
  );
...
addColorArray(
  "Alternate of Black",
  ["hf", "fhf", "gfhy", "gfh", "gfh", "gfh"],
  ["#ff5600", "#00ff66", "#0140ff", "#463245", "#653211", "#312654"]
);
addColorArray(
  "Example Colors",
  ["Color1", "Color2", "Color3"],
  ["#ffffff", "#ff00ff", "#00ffff"]
);
...
function addColorArray(name, names, hexCodes) {
  colorNames.push({ name: name, names: names });
  colorHexCodes.push({ name: name, hexCodes: hexCodes });
}

function showAllColors() {
  clearGallery();
  colorNames.forEach((color, index) => {
    createPart(color.name, colorHexCodes[index].hexCodes, color.names);
  });
}
...
function showSearchResults(searchTerm) {
  ...
  const addedColors = {};
  colorNames.forEach((color, index) => {
    color.names.forEach((name, i) => {
      if (
        (name.toLowerCase().includes(searchTerm) ||
          colorHexCodes[index].hexCodes[i]
            .toLowerCase()
            .includes(searchTerm)) &&
        !addedColors[colorHexCodes[index].hexCodes[i]]
      ) {
        // Check if color already added
        filteredHex.push(colorHexCodes[index].hexCodes[i]);
        filteredNames.push(name);
        addedColors[colorHexCodes[index].hexCodes[i]] = true; // Mark color as added
      }
    });
  });
  if (filteredHex.length > 0) {
    createPart("Search Results", filteredHex, filteredNames);
  } else {
    gallery.innerHTML =
      "<div class='w-100 h-100 d-flex align-items-center justify-content-center'><div class='d-block w-70 text-center'><h5 class='mb-0'><b>Sorry</b>, Color not exist.</h5><p class='fw-light mb-3'><br>If you know about that color, Please contribute here.</p><a href='#about' class='btn btn-primary w-33 px-3 py-2 rounded-pill m-auto border'>Contribute</a></div></div>";
  }
}
...
function createPart(headingName, hexCode, colorName) {
  ...
  for (let i = 0; i < hexCode.length; i++) {
      ...
      parent.postMessage(
        {
          pluginMessage: {
            type: "addToCanvas",
            text: colorName[i] + " - " + hexCode[i],
            value: hexToRgb(hexCode[i]),
          },
        },
        "*"
      );
    ...
    span2.addEventListener("click", () => {
      ...
      parent.postMessage(
        { pluginMessage: { type: "copied", text: hexCode[i] } },
        "*"
      );
    });
